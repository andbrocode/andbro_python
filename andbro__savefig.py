...
def __savefig(figs, outpath=None, outname=None, mode=None):
	''' 
	...
	if type(figs) is list:
		...
		if mode == 'pdf':
			...
			for f in figs: 
				f.savefig(f"{outpath}{outname}.{mode}", dpi=300, facecolor='w', edgecolor='w', orientation='portrait', format='png', transparent=False , bbox_inches='tight', pad_inches=0.1)
		else:
			print('\n   -> mode not correct!')
	...
	## check if figs is a FIGURE OBJECT
		...
		if mode == 'pdf':
			...
			print(f'\n   -> saving {outpath}{outname}.{mode} ...\n')
			figs.savefig(f"{outpath}{outname}.{mode}", dpi=300, facecolor='w', edgecolor='w', orientation='portrait', format='png', transparent=False , bbox_inches='tight', pad_inches=0.1)
