#!/usr/bin/python
#
# compute backazimuth with vertical rotation and transverse acceleration
#
# by AndBro @2023
...
def __compute_backazimuth(st_acc, st_rot, config, event=None, plot=True):
    ...
    ARGUMENTS:
     - st_acc       stream: obspy stream
     - st_rot       stream: obspy stream
     - config       dict: provides important parameters
     - event        bool: provide event as catalog object or event is searched
     - plot         bool: decides if plot is shown

    DEPENDENCIES:
     - ...

    OUTPUT:
     - output       dict: contains output data
     
    EXAMPLE:
    >>> output = __compute_backazimuth(st_acc, st_rot, config, event=None, plot=True)
