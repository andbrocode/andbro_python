...
def __querrySeismoData(seed_id=None, starttime=None, endtime=None, where=None, path=None, restitute=True, detail=None, fill_value=None):
    ...
    if where.lower() in ['george', 'jane']:
        
        if where.lower() == 'george':
            waveform_client = Client(base_url='http://george', timeout=200)
        elif where.lower() == 'jane':
    ...
    try:
        ...
        print("Coordinates could not be attached! Inventory not complete!")
...
#             tr.data = ma.filled(tr.data, fill_value=-999999)
    ...
    ## final trim to exact time window of request
